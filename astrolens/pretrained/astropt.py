"""Optional loader for a released Smith42/astroPT checkpoint into astrolens.models.astropt.AstroPT.

Source: https://huggingface.co/Smith42/astroPT (Smith et al., 2024,
https://arxiv.org/abs/2405.14930), license MIT. Default checkpoint:
`models/fully_trained/0089M_params/030000_ckpt.pt`,
sha256 `3b4450ee8b3ca94ad7b9d632e7998a044f6bbe1548d3ef9cbb2802cd93ad972c`
(verified by `huggingface_hub` against the repo's recorded LFS hash on
download). Requires the optional `huggingface_hub` dependency
(`pip install astrolens[pretrained]`), imported lazily so it is not a core
runtime dependency.
"""
...
def load_pretrained_backbone(model, repo_id: str = REPO_ID, filename: str = CKPT_FILENAME) -> dict:
