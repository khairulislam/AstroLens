...
import astrolens
from astrolens.registry import create_model, is_model, list_models, register_model


def test_known_models_are_registered():
    for name in ["gcnn_d4", "astroformer", "linformer", "astropt"]:
...
def test_astrolens_reexports_registry_api():
    assert astrolens.create_model is create_model
    assert astrolens.is_model is is_model
    assert astrolens.list_models is list_models
    assert astrolens.register_model is register_model
